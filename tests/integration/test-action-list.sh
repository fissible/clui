...
shellframe_test_begin "action-list: confirm with no changes — nothing printed"
out=$(_pty ENTER)
# With all actions on "nothing", no fruit lines should appear in output
assert_contains "$out" "Confirmed!"

shellframe_test_begin "action-list: move to banana, cycle to eat, confirm"
out=$(_pty DOWN SPACE ENTER)
assert_contains "$out" "banana → eat"

shellframe_test_begin "action-list: move to banana, cycle twice to peel, confirm"
out=$(_pty DOWN SPACE SPACE ENTER)
assert_contains "$out" "banana → peel"

shellframe_test_begin "action-list: move to cherry, cycle to eat, confirm"
out=$(_pty DOWN DOWN SPACE ENTER)
assert_contains "$out" "cherry → eat"

shellframe_test_begin "action-list: quit with q — Aborted printed"
out=$(_pty q)
