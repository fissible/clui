#!/usr/bin/env bash
# tests/docker/run-matrix.sh — Run the clui test suite against bash 3.2, 4.4, and 5.x
#
# Usage: bash tests/docker/run-matrix.sh [--no-cache]
#
# Builds one Docker image per bash version (tagged clui-test-bash{3,4,5}),
...
declare -a _TAGS=("clui-test-bash3" "clui-test-bash4" "clui-test-bash5")
...
for _i in "${!_LABELS[@]}"; do
    ...
    # Build
    if ! docker build $NO_CACHE -q -f "$_df" -t "$_tag" "$CLUI_DIR"; then
        printf 'BUILD FAILED\n'
