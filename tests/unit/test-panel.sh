...
shellframe_test_begin "panel_inner: single — top offset by 1"
result=$(_inner 1 1 20 10)
it=$(printf '%s' "$result" | awk '{print $1}')
assert_eq "2" "$it" "inner top = outer top + 1"

shellframe_test_begin "panel_inner: single — left offset by 1"
result=$(_inner 1 1 20 10)
il=$(printf '%s' "$result" | awk '{print $2}')
assert_eq "2" "$il" "inner left = outer left + 1"

shellframe_test_begin "panel_inner: single — width reduced by 2"
result=$(_inner 1 1 20 10)
iw=$(printf '%s' "$result" | awk '{print $3}')
assert_eq "18" "$iw" "inner width = outer width - 2"

shellframe_test_begin "panel_inner: single — height reduced by 2"
result=$(_inner 1 1 20 10)
...
shellframe_test_begin "panel_inner: none — inner equals outer"
...
shellframe_test_begin "panel_inner: non-origin region with single border"
SHELLFRAME_PANEL_STYLE="single"
_ot="" _ol="" _ow="" _oh=""
...
shellframe_test_begin "panel_on_key: always returns 1 (not handled)"
shellframe_panel_on_key "x"
assert_eq "1" "$?" "on_key returns 1"

# ── shellframe_panel_size ─────────────────────────────────────────────────────

shellframe_test_begin "panel_size: min 2x2, preferred unconstrained"
