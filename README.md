...
## Portability and `.toolrc.local`

On first source, `clui.sh` writes a `.toolrc.local` file (gitignored) with
feature flags detected from the current environment:

```bash
# .toolrc.local — auto-generated by clui.sh, do not commit
CLUI_BASH_VERSION=3       # major bash version
CLUI_FD_ALLOC=0           # 1 if {varname}>&1 fd allocation works (bash 4.1+)
CLUI_READ_DECIMAL_T=0     # 1 if read -t 0.1 decimal timeout works (bash 4+)
CLUI_TPUT_OK=1            # 1 if tput is available and functional
```

Subsequent loads source `.toolrc.local` instead of re-detecting. Functions
check these flags to select the right code path for the running bash version.
The key known difference is bash 3.2 (macOS default): no decimal `-t`, no
`{varname}` fd allocation, and subtly different `printf` behavior (all
documented in [Hard-won lessons](#hard-won-lessons)).
...
├── .toolrc.local    # auto-generated feature flags (gitignored)
├── src/
